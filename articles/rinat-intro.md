...
rinat is a wrapper for iNaturalist APIs for accessing the observations.
The detailed documentation of the API is available on the [iNaturalist
...
is the primary function that retrieves observations from iNaturalist.
The text or taxon search can be refined by observation date, record
...
return entries that *mention* Monarch butterflies, not just Monarch
observations.
...
> Note that
> [`get_inat_obs()`](https://docs.ropensci.org/rinat/reference/get_inat_obs.md)
> will return 100 observations by default. This can be controlled with
> the `maxresults` argument.

Another use for a fuzzy search is searching for a habitat,
e.g. searching for all observations that might happen in a [vernal
pool](https://en.wikipedia.org/wiki/Vernal_pool). We can then explore
the taxon names found.
...
![plot of chunk
unnamed-chunk-5](unnamed-chunk-5-1.png "plot of chunk unnamed-chunk-5")
...
Basic maps can be created with the
[`inat_map()`](https://docs.ropensci.org/rinat/reference/inat_map.md)
function to quickly visualize search results. The `plot = FALSE` option
can be used to avoid displaying the initial plot when further
...
![plot of chunk
unnamed-chunk-13](unnamed-chunk-13-1.png "plot of chunk unnamed-chunk-13")

[`inat_map()`](https://docs.ropensci.org/rinat/reference/inat_map.md) is
useful for quickly mapping single-species data obtained with rinat.
However, more complicated plots are best made from scratch. Here is an
example of customised map that does not make use of it. (Note the use of
`quality = "research"` to restrict the search to the more reliable
observations.)
...
![plot of chunk
unnamed-chunk-14](unnamed-chunk-14-1.png "plot of chunk unnamed-chunk-14")
